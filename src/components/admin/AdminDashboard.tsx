...
export const AdminDashboard: React.FC = () => {
  ...
  const filteredDevices = allDevices.filter((d) => {
    if (filterStatus !== 'all' && d.connectionStatus !== filterStatus) return false;
    if (
      deviceSearch &&
  ...
  const totalConnected = allDevices.filter((d) => d.connectionStatus === 'connected').length;
  const totalOffline = allDevices.filter((d) => d.connectionStatus === 'offline').length;
  ...
  return (
    <div className="flex-1 bg-slate-950 text-white min-h-[calc(100vh-60px)] p-4 sm:p-6 flex flex-col">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 p-4 rounded-3xl mb-6 shadow-xl">
        ...
        <div className="flex items-center gap-3 self-start md:self-auto">
          <div className="px-3.5 py-2 rounded-xl bg-slate-950/80 border border-slate-800 text-center">
            ...
            <div className="text-sm font-bold text-white mt-0.5">{allDevices.length}</div>
          </div>
          <div className="px-3.5 py-2 rounded-xl bg-slate-950/80 border border-slate-800 text-center">
            <div className="text-[10px] uppercase font-bold text-emerald-400">Online</div>
            <div className="text-sm font-bold text-emerald-400 mt-0.5">{totalConnected}</div>
          </div>
          <div className="px-3.5 py-2 rounded-xl bg-slate-950/80 border border-slate-800 text-center">
      ...
      {/* Main Grid: Device Fleet Column + Command Center Column */}
